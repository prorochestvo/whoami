...
2. Verify completion gates before moving — run both:
   - `make build` — the generator must succeed and produce `build/` (this also runs `go fmt` + `go vet`).
   - `make test` — `go fmt` + `go vet` + `go test ./...`.
   If the project lacks a Makefile, run the equivalent `go` commands directly. If anything fails, stop and report failures. Do **not** move the file.
